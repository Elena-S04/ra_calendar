...
function CalendarFunc(props) {
  ...
  const weekdayBeginingMonth = function (date) {
    // число дня недели (день недели) с которого начинаеться месяц
  ...
  Array.from({ length: weekdayBeginingMonth(beginingMonthDate) }).forEach(
    () => {
      // добавление дней предыдущего месяца в массив элементов календаря
      classTd = "ui-datepicker-other-month";
      calendarNotes.unshift(
        <td className={classTd}>{(daysPrevMonthCount -= 1)}</td>
      );
    }
  );

  Array.from({ length: daysInMonth(date) }).forEach((item, index) => {
    // добавление дней текущего месяца в массив элементов каледнаря
    index === date.getDate() - 1
      ? (classTd = "ui-datepicker-today")
      : (classTd = "");
    calendarNotes.push(<td className={classTd}>{index + 1}</td>);
  });

  const daysNextMonth =
    weekdayBeginingMonth(nextMonthDate) === 0
      ? 0
      : 7 - weekdayBeginingMonth(nextMonthDate); // количество дней следующего месяца попадпющих в календарь

  Array.from({ length: daysNextMonth }).forEach((item, index) => {
    // добавление дней следующего месяца в массив элементов календаря
    classTd = "ui-datepicker-other-month";
    calendarNotes.push(<td className={classTd}>{index + 1}</td>);
  });
  ...
  return (
    <div className="ui-datepicker">
      ...
      <table className="ui-datepicker-calendar">
        ...
        <tbody>
          <tr>{calendarNotes.slice(0, 7)}</tr>
          <tr>{calendarNotes.slice(7, 14)}</tr>
          <tr>{calendarNotes.slice(14, 21)}</tr>
          <tr>{calendarNotes.slice(21, 28)}</tr>
          <tr>{calendarNotes.slice(28, 35)}</tr>
        </tbody>
      </table>
    </div>
  );
